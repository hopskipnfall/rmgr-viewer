...
/**
 * 6. DARK SAMUS (0x22): Phazon-corrupted dark armor, cyan glowing energy
 * veins, sharp spiked shoulders, sinister visor slit.
 */
export function drawDarkSamusPolygons(
  ctx: CanvasRenderingContext2D,
  backgroundTheme: BackgroundTheme,
  x: number,
  y: number,
  ...
  const baseSuit = isMountain ? "#09090b" : isAutumn ? "#1c1917" : "#18181b";
  const baseVeins = isMountain ? "#a855f7" : isAutumn ? "#06b6d4" : "#00f2fe";
  const baseGlow = isMountain ? "#ec4899" : isAutumn ? "#38bdf8" : "#38bdf8";

  let suit = resolveColor(baseSuit, isOpponent);
  let veins = resolveColor(baseVeins, isOpponent);
  const glow = resolveColor(baseGlow, isOpponent);
  const outline = resolveColor("rgba(0, 242, 254, 0.4)", isOpponent);

  if (taunting) {
    const hue = (actionFrameCounter * 10) % 360;
    veins = resolveColor(`hsl(${hue}, 95%, 60%)`, isOpponent);
  } else if (isRoll) {
    suit = resolveColor(baseSuit, isOpponent, 0.45);
    veins = resolveColor(baseVeins, isOpponent, 0.45);
  }
  ...
  // Armored legs with sharp knee spikes
  ctx.beginPath();
  ctx.moveTo(posX - 0.2 * dir * w, y - 0.45 * h);
  ctx.lineTo(posX - 0.35 * dir * w, y);
  ctx.lineTo(posX + 0.35 * dir * w, y);
  ctx.lineTo(posX + 0.2 * dir * w, y - 0.45 * h);
  ctx.closePath();
  ctx.fillStyle = suit;
  ctx.fill();
  ctx.strokeStyle = outline;
  ...
  // Torso / Varia shell corrupted with Phazon
  ctx.beginPath();
  ctx.moveTo(posX - 0.35 * dir * w, y - 0.72 * h);
  ...
  // Spiked Phazon Shoulder Pauldrons
  ctx.beginPath();
  ctx.moveTo(posX - 0.25 * dir * w, y - 0.75 * h);
  ctx.lineTo(posX - 0.65 * dir * w, y - 0.88 * h); // Sharp shoulder spire
  ctx.lineTo(posX - 0.35 * dir * w, y - 0.62 * h);
  ctx.closePath();
  ctx.fillStyle = suit;
  ctx.fill();
  ctx.stroke();
  ...
  ctx.beginPath();
  ctx.moveTo(posX + 0.25 * dir * w, y - 0.75 * h);
  ctx.lineTo(posX + 0.65 * dir * w, y - 0.88 * h);
  ctx.lineTo(posX + 0.35 * dir * w, y - 0.62 * h);
  ctx.closePath();
  ctx.fillStyle = suit;
  ctx.fill();
  ...
  // Glowing Phazon energy veins across chest
  ctx.beginPath();
  ctx.moveTo(posX - 0.2 * dir * w, y - 0.65 * h);
  ctx.lineTo(posX, y - 0.5 * h);
  ctx.lineTo(posX + 0.2 * dir * w, y - 0.65 * h);
  ctx.strokeStyle = veins;
  ctx.lineWidth = 2.2;
  ctx.stroke();

  // Arm cannon pointing forward
  ctx.beginPath();
  ctx.rect(posX + 0.3 * dir * w, y - 0.52 * h, 0.35 * dir * w, 0.12 * h);
  ctx.fillStyle = suit;
  ctx.fill();
  ctx.stroke();

  // Arm cannon muzzle glow
  ctx.beginPath();
  ctx.arc(
    posX + 0.65 * dir * w,
    y - 0.46 * h,
  ...
  ctx.lineTo(posX + 0.05 * dir * w, y - 0.76 * h);
  ctx.closePath();
  ctx.fillStyle = veins;
  ctx.fill();
...
/**
 * 7. LUCAS (0x26): Blonde swept hair quiff, yellow/orange striped shirt,
 * blue shorts, red sneakers, PSI glow.
 */
export function drawLucasPolygons(
  ctx: CanvasRenderingContext2D,
  backgroundTheme: BackgroundTheme,
  x: number,
  y: number,
  ...
  const baseHair = isMountain ? "#fef08a" : isAutumn ? "#fbbf24" : "#fde047";
  const baseYellow = isMountain ? "#fef9c3" : isAutumn ? "#fde68a" : "#facc15";
  const baseOrange = isMountain ? "#f43f5e" : isAutumn ? "#ea580c" : "#f97316";
  const baseShorts = isMountain ? "#3b82f6" : isAutumn ? "#1e40af" : "#2563eb";
  const baseShoes = isMountain ? "#ef4444" : isAutumn ? "#dc2626" : "#ef4444";
  ...
  if (taunting) {
    const hue = (actionFrameCounter * 10) % 360;
    orange = resolveColor(`hsl(${hue}, 90%, 55%)`, isOpponent);
  } else if (isRoll) {
    hair = resolveColor(baseHair, isOpponent, 0.45);
    yellow = resolveColor(baseYellow, isOpponent, 0.45);
  ...
  // Blue shorts
  ctx.beginPath();
  ctx.rect(posX - 0.25 * dir * w, y - 0.35 * h, 0.5 * dir * w, 0.18 * h);
  ctx.fillStyle = shorts;
  ctx.fill();
  ctx.stroke();

  // Striped Yellow & Orange Shirt
  ctx.beginPath();
  ctx.rect(posX - 0.3 * dir * w, y - 0.65 * h, 0.6 * dir * w, 0.3 * h);
  ctx.fillStyle = yellow;
  ctx.fill();
  ctx.stroke();
  ...
  ctx.fillRect(posX - 0.3 * dir * w, y - 0.58 * h, 0.6 * dir * w, 0.06 * h);
  ctx.fillRect(posX - 0.3 * dir * w, y - 0.46 * h, 0.6 * dir * w, 0.06 * h);

  // Round boyish head
  ctx.beginPath();
  ctx.arc(posX, y - 0.75 * h, Math.max(1, 0.28 * w), 0, Math.PI * 2);
  ctx.fillStyle = skin;
  ctx.fill();
  ctx.stroke();
  ...
  // Iconic Swept-up Blonde Hair Quiff
  ctx.beginPath();
  ctx.moveTo(posX - 0.25 * dir * w, y - 0.85 * h);
  ctx.quadraticCurveTo(
    posX + 0.1 * dir * w,
    y - 1.15 * h,
    posX + 0.38 * dir * w,
...
/**
 * 8. GIGA BOWSER (0x35): Colossal demon King of the Koopas with sweeping
 * curved horns, jagged maw, obsidian spiky carapace, and fiery red glow.
 */
export function drawGigaBowserPolygons(
  ctx: CanvasRenderingContext2D,
  backgroundTheme: BackgroundTheme,
  x: number,
  y: number,
  ...
  const baseScales = isMountain ? "#0f172a" : isAutumn ? "#1c1917" : "#18181b";
  const baseShell = isMountain ? "#450a0a" : isAutumn ? "#431407" : "#3f1a0a";
  const baseRim = isMountain ? "#f87171" : isAutumn ? "#ea580c" : "#dc2626";
  const baseSpikes = isMountain ? "#e2e8f0" : isAutumn ? "#fef3c7" : "#f1f5f9";
  const baseHair = isMountain ? "#ef4444" : isAutumn ? "#b91c1c" : "#991b1b";

  let scales = resolveColor(baseScales, isOpponent);
  let shell = resolveColor(baseShell, isOpponent);
  let rim = resolveColor(baseRim, isOpponent);
  const spikes = resolveColor(baseSpikes, isOpponent);
  const hair = resolveColor(baseHair, isOpponent);
  const outline = resolveColor("rgba(0, 0, 0, 0.9)", isOpponent);

  if (taunting) {
    const hue = (actionFrameCounter * 10) % 360;
    rim = resolveColor(`hsl(${hue}, 90%, 55%)`, isOpponent);
  } else if (isRoll) {
    scales = resolveColor(baseScales, isOpponent, 0.45);
    shell = resolveColor(baseShell, isOpponent, 0.45);
    rim = resolveColor(baseRim, isOpponent, 0.45);
  }
  ...
  ctx.lineTo(posX - 1.1 * dir * w, y - 0.12 * h);
  ctx.lineTo(posX - 0.25 * dir * w, y - 0.1 * h);
  ctx.closePath();
  ...
  ctx.fillStyle = rim; // Glowing red/lava rim
  ctx.fill();
  ctx.stroke();
  ...
  ctx.fillStyle = shell; // Dark shell dome
  ctx.fill();
  ctx.stroke();
  ...
  for (const [sx, sy] of [
    ...
    ctx.beginPath();
    ctx.moveTo(posX + (sx + 0.1) * dir * w, y - sy * h);
    ctx.lineTo(posX + (sx - 0.25) * dir * w, y - (sy + 0.15) * h);
    ctx.lineTo(posX + (sx - 0.1) * dir * w, y - (sy - 0.1) * h);
    ctx.closePath();
  ...
  // Broad Demonic Torso & Spiked Forearms
  ctx.beginPath();
  ctx.moveTo(posX - 0.1 * dir * w, y - 0.72 * h);
  ctx.lineTo(posX + 0.45 * dir * w, y - 0.72 * h);
  ctx.lineTo(posX + 0.35 * dir * w, y - 0.25 * h);
  ctx.lineTo(posX - 0.1 * dir * w, y - 0.25 * h);
  ctx.closePath();
  ctx.fillStyle = scales;
  ctx.fill();
  ctx.stroke();

  // Head & Jagged Fanged Maw
  ctx.beginPath();
  ctx.moveTo(posX - 0.1 * dir * w, y - 0.75 * h);
  ctx.lineTo(posX + 0.75 * dir * w, y - 0.75 * h);
  ctx.lineTo(posX + 0.5 * dir * w, y - 0.62 * h);
  ...
  // Giant Razor Teeth
  ctx.beginPath();
  ctx.moveTo(posX + 0.4 * dir * w, y - 0.75 * h);
  ctx.lineTo(posX + 0.46 * dir * w, y - 0.68 * h);
  ctx.lineTo(posX + 0.52 * dir * w, y - 0.75 * h);
  ctx.lineTo(posX + 0.58 * dir * w, y - 0.68 * h);
  ctx.lineTo(posX + 0.64 * dir * w, y - 0.75 * h);
  ctx.fillStyle = spikes;
  ctx.fill();

  // Sweeping Demonic Bull Horns
  ctx.beginPath();
  ctx.moveTo(posX, y - 0.9 * h);
  ctx.quadraticCurveTo(
    posX - 0.3 * dir * w,
    y - 1.35 * h,
    posX - 0.65 * dir * w,
