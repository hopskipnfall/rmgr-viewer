...
/**
 * 31. LANKY KONG (0x4c):
 * Donkey Kong 64 orangutan with comical ultra-long stretching arms,
 * blue denim overalls, red clown nose, and goofy wide grin.
 */
export function drawLankyKongPolygons(
  ctx: CanvasRenderingContext2D,
  backgroundTheme: BackgroundTheme,
  x: number,
  y: number,
  ...
  if (taunting) {
    const armWave =
      Math.sin(actionFrameCounter * 0.3) * 0.18 * (facingRight ? 1 : -1);
    ctx.translate(posX, centerY);
    ctx.rotate(armWave);
    ctx.translate(-posX, -centerY);
  }
  ...
  const baseOveralls = isMountain
    ? "#1d4ed8"
    : isAutumn
      ? "#1e3a8a"
      : "#2563eb";
  const baseShirt = isMountain ? "#f1f5f9" : isAutumn ? "#fef08a" : "#ffffff";
  const baseNose = isMountain ? "#dc2626" : isAutumn ? "#b91c1c" : "#ef4444";
  const baseMuzzle = isMountain ? "#fed7aa" : isAutumn ? "#ffedd5" : "#fdba74";

  let fur = resolveColor(baseFur, isOpponent);
  let overalls = resolveColor(baseOveralls, isOpponent);
  const shirt = resolveColor(baseShirt, isOpponent);
  let nose = resolveColor(baseNose, isOpponent);
  const muzzle = resolveColor(baseMuzzle, isOpponent);
  const outline = resolveColor("rgba(15, 23, 42, 0.8)", isOpponent);

  if (taunting) {
    const noseHue = (actionFrameCounter * 16) % 360;
    nose = resolveColor(`hsl(${noseHue}, 95%, 55%)`, isOpponent);
  } else if (isRoll) {
    fur = resolveColor(baseFur, isOpponent, 0.45);
    overalls = resolveColor(baseOveralls, isOpponent, 0.45);
  }
  ...
  // Ridiculously long orangutan arms extending down past knees!
  ctx.beginPath();
  // Rear long arm
  ctx.moveTo(posX - 0.32 * dir * w, y - 0.65 * h);
  ctx.lineTo(posX - 0.65 * dir * w, y - 0.25 * h);
  ctx.lineTo(posX - 0.68 * dir * w, y + 0.05 * h); // Big hands touching ground
  ctx.strokeStyle = fur;
  ctx.lineWidth = Math.max(3, 0.1 * w);
  ctx.stroke();

  // Front long arm
  ctx.beginPath();
  ctx.moveTo(posX + 0.28 * dir * w, y - 0.65 * h);
  ctx.lineTo(posX + 0.62 * dir * w, y - 0.25 * h);
  ctx.lineTo(posX + 0.65 * dir * w, y + 0.05 * h);
  ctx.strokeStyle = fur;
  ctx.lineWidth = Math.max(3, 0.1 * w);
  ctx.stroke();
  ...
  // Short ape feet
  ctx.beginPath();
  ctx.moveTo(posX - 0.28 * dir * w, y - 0.15 * h);
  ctx.lineTo(posX - 0.12 * dir * w, y);
  ctx.lineTo(posX - 0.38 * dir * w, y);
  ctx.closePath();
  ctx.fillStyle = fur;
  ...
  ctx.beginPath();
  ctx.moveTo(posX + 0.08 * dir * w, y - 0.15 * h);
  ctx.lineTo(posX + 0.38 * dir * w, y);
  ctx.lineTo(posX + 0.15 * dir * w, y);
  ctx.closePath();
  ctx.fillStyle = fur;
  ...
  // Blue Overalls
  ctx.beginPath();
  ctx.moveTo(posX - 0.3 * dir * w, y - 0.45 * h);
  ctx.lineTo(posX + 0.3 * dir * w, y - 0.45 * h);
  ctx.lineTo(posX + 0.28 * dir * w, y - 0.18 * h);
  ctx.lineTo(posX - 0.28 * dir * w, y - 0.18 * h);
  ctx.closePath();
  ...
  // Overall straps & golden buttons
  ctx.beginPath();
  ctx.moveTo(posX - 0.2 * dir * w, y - 0.68 * h);
  ctx.lineTo(posX - 0.16 * dir * w, y - 0.45 * h);
  ctx.moveTo(posX + 0.2 * dir * w, y - 0.68 * h);
  ctx.lineTo(posX + 0.16 * dir * w, y - 0.45 * h);
  ctx.strokeStyle = overalls;
  ctx.lineWidth = Math.max(2, 0.06 * w);
  ctx.stroke();
  ...
  // White T-shirt chest
  ctx.beginPath();
  ctx.moveTo(posX - 0.25 * dir * w, y - 0.68 * h);
  ...
  // Lanky Ape Head & Big Ears
  ctx.beginPath();
  ctx.arc(posX + 0.05 * dir * w, y - 0.76 * h, 0.24 * w, 0, Math.PI * 2);
  ctx.fillStyle = fur;
  ctx.fill();

  // Rounded ape ears
  ctx.beginPath();
  ctx.arc(posX - 0.15 * dir * w, y - 0.78 * h, 0.07 * w, 0, Math.PI * 2);
  ctx.arc(posX + 0.25 * dir * w, y - 0.78 * h, 0.07 * w, 0, Math.PI * 2);
  ctx.fillStyle = muzzle;
  ctx.fill();

  // Goofy wide muzzle & grinning mouth
  ctx.beginPath();
  ctx.ellipse(
    posX + 0.12 * dir * w,
    y - 0.7 * h,
  ...
  // Toothy goofy grin line
  ctx.beginPath();
  ctx.moveTo(posX - 0.02 * dir * w, y - 0.68 * h);
  ctx.lineTo(posX + 0.26 * dir * w, y - 0.68 * h);
  ctx.strokeStyle = "#451a03";
  ctx.lineWidth = 2.0;
  ctx.stroke();

  // Big Bright Red Clown Nose!
  ctx.beginPath();
  ctx.arc(
    posX + 0.16 * dir * w,
    y - 0.76 * h,
    Math.max(2.5, 0.065 * w),
    0,
    Math.PI * 2,
  );
  ctx.fillStyle = nose;
  ...
  // Player emblem badge
  ctx.beginPath();
  ctx.arc(posX, y - 0.38 * h, Math.max(1.5, 0.06 * w), 0, Math.PI * 2);
  ctx.fillStyle = playerColor;
  ctx.fill();
