...
/**
 * 21. SANDBAG (0x3c):
 * Canvas punchbag with burlap texture, stitched seams, grommet top knot,
 * innocent black bead eyes, and comical squash/stretch.
 */
export function drawSandbagPolygons(
  ctx: CanvasRenderingContext2D,
  backgroundTheme: BackgroundTheme,
  x: number,
  y: number,
  ...
  // Sandbag wobble or comical squash
  let scaleX = 1.0;
  let scaleY = 1.0;
  if (inCombo) {
    scaleX = 1.15;
    ...
    const hop = Math.sin(actionFrameCounter * 0.4);
    scaleX = 1.0 + hop * 0.08;
    scaleY = 1.0 - hop * 0.08;
  }
  ...
  const baseSack = isMountain ? "#d4d4d8" : isAutumn ? "#d97706" : "#e4d5b7";
  const baseShade = isMountain ? "#a1a1aa" : isAutumn ? "#b45309" : "#cbb692";
  const baseSeam = isMountain ? "#52525b" : isAutumn ? "#78350f" : "#8c7355";
  const baseGrommet = isMountain ? "#94a3b8" : isAutumn ? "#facc15" : "#cbd5e1";

  let sack = resolveColor(baseSack, isOpponent);
  let shade = resolveColor(baseShade, isOpponent);
  const seam = resolveColor(baseSeam, isOpponent);
  let grommet = resolveColor(baseGrommet, isOpponent);
  const outline = resolveColor("rgba(15, 23, 42, 0.8)", isOpponent);

  if (taunting) {
    const grommetHue = (actionFrameCounter * 16) % 360;
    grommet = resolveColor(`hsl(${grommetHue}, 90%, 55%)`, isOpponent);
  } else if (isRoll) {
    sack = resolveColor(baseSack, isOpponent, 0.45);
    shade = resolveColor(baseShade, isOpponent, 0.45);
  }
  ...
  // Main pear-shaped / cylindrical canvas sack body
  ctx.beginPath();
  ctx.moveTo(posX - 0.25 * w, y - 0.9 * h);
  ctx.bezierCurveTo(
    posX - 0.35 * w,
    y - 0.55 * h,
    posX - 0.48 * w,
  ...
  // Darker shadow half on rear side
  ctx.beginPath();
  ...
  ctx.lineTo(posX - 0.35 * dir * w, y - 0.55 * h);
  ctx.closePath();
  ctx.fillStyle = shade;
  ctx.fill();
  ...
  // Top tied bunch / knot
  ctx.beginPath();
  ctx.moveTo(posX - 0.18 * w, y - 0.9 * h);
  ctx.lineTo(posX - 0.25 * w, y - 1.02 * h);
  ctx.lineTo(posX + 0.25 * w, y - 1.02 * h);
  ctx.lineTo(posX + 0.18 * w, y - 0.9 * h);
  ctx.closePath();
  ctx.fillStyle = shade;
  ctx.fill();
  ctx.strokeStyle = outline;
  ...
  // Metal grommet / ring at top
  ctx.beginPath();
  ctx.arc(posX, y - 0.96 * h, Math.max(2, 0.08 * w), 0, Math.PI * 2);
  ctx.fillStyle = grommet;
  ctx.fill();
  ctx.beginPath();
  ctx.arc(posX, y - 0.96 * h, Math.max(1, 0.04 * w), 0, Math.PI * 2);
  ctx.fillStyle = "#334155";
  ctx.fill();

  // Double stitched side seam
  ctx.beginPath();
  ctx.setLineDash([2, 3]);
  ctx.moveTo(posX - 0.05 * dir * w, y - 0.88 * h);
  ctx.lineTo(posX + 0.1 * dir * w, y - 0.1 * h);
  ctx.strokeStyle = seam;
  ctx.lineWidth = 1.5;
  ctx.stroke();
  ctx.setLineDash([]);

  // Innocent cartoon black bead eyes with shine dots
  ctx.beginPath();
  ctx.arc(
    posX + 0.15 * dir * w,
    y - 0.65 * h,
  ...
  ctx.beginPath();
  ctx.arc(posX, y - 0.35 * h, Math.max(2, 0.08 * w), 0, Math.PI * 2);
  ctx.fillStyle = playerColor;
  ctx.fill();
...
/**
 * 22. MAD PIANO (0x36):
 * Super Mario 64 haunted grand piano with sharp predator teeth,
 * red velvet maw, glossy black cabinet, brass pedals, and snapping lid.
 */
export function drawPianoPolygons(
  ctx: CanvasRenderingContext2D,
  backgroundTheme: BackgroundTheme,
  x: number,
  y: number,
  ...
  // Snapping lid animation
  let jawSnap = 0.0;
  if (inCombo || taunting) {
    jawSnap = Math.abs(Math.sin(actionFrameCounter * 0.45)) * 0.15;
  }

  const isMountain = backgroundTheme === "mountain";
  const isAutumn = backgroundTheme === "autumn";

  const baseBlack = isMountain ? "#09090b" : isAutumn ? "#18181b" : "#0f172a";
  const baseHighlight = isMountain
    ? "#27272a"
    : isAutumn
      ? "#3f3f46"
      : "#334155";
  const baseMaw = isMountain ? "#991b1b" : isAutumn ? "#7f1d1d" : "#b91c1c";
  const baseTeeth = isMountain ? "#f1f5f9" : isAutumn ? "#fef3c7" : "#ffffff";
  const baseBrass = isMountain ? "#f59e0b" : isAutumn ? "#d97706" : "#fbbf24";

  let pianoBody = resolveColor(baseBlack, isOpponent);
  const highlight = resolveColor(baseHighlight, isOpponent);
  let maw = resolveColor(baseMaw, isOpponent);
  let teeth = resolveColor(baseTeeth, isOpponent);
  const brass = resolveColor(baseBrass, isOpponent);
  const outline = resolveColor("rgba(15, 23, 42, 0.9)", isOpponent);

  if (taunting) {
    const teethHue = (actionFrameCounter * 15) % 360;
    teeth = resolveColor(`hsl(${teethHue}, 90%, 65%)`, isOpponent);
  } else if (isRoll) {
    pianoBody = resolveColor(baseBlack, isOpponent, 0.45);
    maw = resolveColor(baseMaw, isOpponent, 0.45);
  }
  ...
  // Piano Legs (three wooden tapering posts)
  ctx.beginPath();
  // Rear leg
  ctx.moveTo(posX - 0.45 * dir * w, y - 0.4 * h);
  ctx.lineTo(posX - 0.42 * dir * w, y);
  ctx.lineTo(posX - 0.35 * dir * w, y);
  ctx.lineTo(posX - 0.38 * dir * w, y - 0.4 * h);
  // Center leg with brass caster
  ctx.moveTo(posX - 0.05 * dir * w, y - 0.4 * h);
  ctx.lineTo(posX - 0.03 * dir * w, y);
  ctx.lineTo(posX + 0.03 * dir * w, y);
  ctx.lineTo(posX + 0.05 * dir * w, y - 0.4 * h);
  // Front leg
  ctx.moveTo(posX + 0.38 * dir * w, y - 0.4 * h);
  ctx.lineTo(posX + 0.35 * dir * w, y);
  ctx.lineTo(posX + 0.42 * dir * w, y);
  ctx.lineTo(posX + 0.45 * dir * w, y - 0.4 * h);
  ctx.closePath();
  ctx.fillStyle = pianoBody;
  ctx.fill();

  // Brass Lyre / Pedals
  ctx.beginPath();
  ctx.arc(posX, y - 0.05 * h, Math.max(2, 0.06 * w), 0, Math.PI * 2);
  ctx.fillStyle = brass;
  ctx.fill();

  // Lower Piano Cabinet / Bed
  ctx.beginPath();
  ctx.moveTo(posX - 0.55 * dir * w, y - 0.55 * h);
  ctx.lineTo(posX + 0.52 * dir * w, y - 0.48 * h);
  ctx.lineTo(posX + 0.48 * dir * w, y - 0.38 * h);
  ctx.lineTo(posX - 0.52 * dir * w, y - 0.42 * h);
  ctx.closePath();
  ctx.fillStyle = pianoBody;
  ctx.fill();
  ctx.strokeStyle = outline;
  ...
  // Deep Crimson Velvet Mouth / Interior
  ctx.beginPath();
  ctx.moveTo(posX - 0.5 * dir * w, y - 0.52 * h);
  ctx.lineTo(posX + 0.5 * dir * w, y - (0.75 + jawSnap) * h);
  ctx.lineTo(posX + 0.52 * dir * w, y - 0.48 * h);
  ctx.closePath();
  ctx.fillStyle = maw;
  ctx.fill();

  // Open Propped Grand Piano Lid
  ctx.beginPath();
  ctx.moveTo(posX - 0.52 * dir * w, y - 0.58 * h);
  ctx.lineTo(posX + 0.48 * dir * w, y - (0.85 + jawSnap) * h);
  ctx.lineTo(posX + 0.44 * dir * w, y - (0.92 + jawSnap) * h);
  ctx.lineTo(posX - 0.54 * dir * w, y - 0.65 * h);
  ctx.closePath();
  ...
  // Brass Lid Prop Stick
  ctx.beginPath();
  ctx.moveTo(posX + 0.25 * dir * w, y - 0.48 * h);
  ctx.lineTo(posX + 0.3 * dir * w, y - (0.85 + jawSnap) * h);
  ctx.strokeStyle = brass;
  ctx.lineWidth = 2.0;
  ctx.stroke();

  // Razor-sharp predatory upper teeth lining the lid!
  ctx.beginPath();
  for (let i = 0; i < 5; i++) {
    const t = i / 4.0;
    const tx = posX + (0.05 + 0.4 * t) * dir * w;
    const ty = y - (0.58 + (0.24 + jawSnap) * t) * h;
    ctx.moveTo(tx, ty);
    ctx.lineTo(tx + 0.04 * dir * w, ty + 0.08 * h);
    ctx.lineTo(tx + 0.08 * dir * w, ty);
  }
  ctx.closePath();
  ctx.fillStyle = teeth;
  ctx.fill();

  // Lower keyboard teeth
  ctx.beginPath();
  for (let i = 0; i < 5; i++) {
    const t = i / 4.0;
    const tx = posX + (0.05 + 0.4 * t) * dir * w;
    const ty = y - 0.48 * h;
    ctx.moveTo(tx, ty);
    ctx.lineTo(tx + 0.04 * dir * w, ty - 0.07 * h);
    ctx.lineTo(tx + 0.08 * dir * w, ty);
  }
  ctx.closePath();
  ctx.fillStyle = teeth;
