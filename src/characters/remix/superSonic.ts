...
/**
 * 15. SUPER SONIC (0x3D): Golden hedgehog, upturned golden quills,
 * ruby red eyes, sparkling golden aura.
 */
export function drawSuperSonicPolygons(
  ctx: CanvasRenderingContext2D,
  backgroundTheme: BackgroundTheme,
  x: number,
  y: number,
  ...
  const baseGold = isMountain ? "#fef08a" : isAutumn ? "#facc15" : "#facc15";
  const baseRed = isMountain ? "#ef4444" : isAutumn ? "#dc2626" : "#ef4444";
  const baseEye = isMountain ? "#f43f5e" : isAutumn ? "#e11d48" : "#dc2626";

  let gold = resolveColor(baseGold, isOpponent);
  let red = resolveColor(baseRed, isOpponent);
  const eye = resolveColor(baseEye, isOpponent);

  if (taunting) {
    const hue = (actionFrameCounter * 10) % 360;
    gold = resolveColor(`hsl(${hue}, 95%, 65%)`, isOpponent);
  } else if (isRoll) {
    gold = resolveColor(baseGold, isOpponent, 0.45);
    red = resolveColor(baseRed, isOpponent, 0.45);
  }
  ...
  // Upturned Golden Quills pointing skyward
  for (const [qx, qy] of [
    [-0.2, 0.85],
    [-0.1, 0.95],
    [0.05, 1.05],
    ...
    ctx.beginPath();
    ctx.moveTo(posX + (qx - 0.1) * dir * w, y - (qy - 0.05) * h);
    ctx.lineTo(posX + (qx - 0.25) * dir * w, y - (qy + 0.25) * h); // Upturned quill tip
    ctx.lineTo(posX + (qx + 0.1) * dir * w, y - (qy - 0.05) * h);
    ctx.closePath();
    ctx.fillStyle = gold;
    ctx.fill();
    ctx.stroke();
